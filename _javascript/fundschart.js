...
const width_main_chart = Math.min(
  main_content_width - main_chart_margin_left - main_chart_margin_right,
  main_chart_width_max
);
...
const layout_main_chart = new (function () {
  ...
  layout_core_chart = new (function () {
    ...
    this.x = this.margin.left;
    this.y =
      layout_top_line.y + layout_top_line.h + layout_top_line.margin.bottom;
    this.w = layout_main_chart.w;
    this.h =
      layout_main_chart.h -
      (layout_top_line.margin.top +
        layout_top_line.margin.bottom +
        layout_top_line.h) -
      (layout_minimap.margin.top +
        layout_minimap.margin.top +
        layout_minimap.h);
  })(),
  layout = new (function () {
...
const x_nticks = function (range, d3_interval) {
  ...
  if (range[1] < d3_interval.offset(range[0], n_ticks_thres))
    return d3_interval;
  else return n_ticks_thres;
};

const zoom_periods = ['YTD', '1M', '3M', '6M', '1Y', '3Y', '5Y', 'All'];

//Read the data
let load_data = d3.csv(
  `${window?.site_baseurl || ''}/assets/data/resampled/W-MON/combine_resampled_data.csv`,
  function (data) {
...
function draw_chart(data, dom_id, chart_name) {
  ...
  let selected_range_text = top_line
    ...
    .attr(
      'transform',
      `translate(${mode_portrait ? 0 : layout_top_line.w}, ${mode_portrait ? 25 : 0})`
    );

  // CORE CHART
  let core_chart = main_chart
    .append('g')
    .classed('core-chart-area', true)
    .attr(
      'transform',
      `translate(${layout_core_chart.x}, ${layout_core_chart.y})`
    );
  ...
  // Core chart X axis
  let x_axis_main = (x_scale) =>
    d3
      .axisBottom(x_scale)
      .ticks(x_nticks(x_scale.domain(), d3.utcMonth))
      .tickFormat(time_format_yymm);
  let core_axis_x = core_chart
    .append('g')
    .classed('core-axis-x', true)
    .attr(
      'transform',
      `translate(${layout_core_chart.x}, ${layout_core_chart.h})`
    )
    .call(x_axis_main(x));
  ...
  function mousemove_main(event) {
    ...
    if (x_selected + 10 < layout_core_chart.w - tooltips_width)
      ...
      tooltips.attr(
        'transform',
        `translate(${x_selected - tooltips_width - 10}, 0)`
      );
    else
  ...
  function mouseout_main() {
    ...
    core_chart.select('.mouse-line').attr('stroke-opacity', 0);
  }
  ...
  let minimap = main_chart
    .insert('g', '.core-chart-area')
  ...
  const brush = d3
    ...
    .extent([
      [0, 0],
      [layout_core_chart.w, layout_minimap.h - 0]
    ])
    .handleSize(12)
  ...
  // LEGENDS
  // Add legends
  let legend_cols = [['Index', 'ETF'], ['Active'], ['Bond']];
  ...
  let legends_items = legends_companies
    ...
    .each(function (d) {
      if (selected_funds.includes(d.name)) this.checked = true;
    });
  ...
  function highlight_fund(fund) {
    let already_highlighted = core_chart
      .select('.core-chart-line-highlight')
      .empty();
    ...
    if (!selected_line.empty()) {
      ...
      legend.select('.legend-color').style('border-radius', '0');
  ...
  function unhighlight_fund(fund) {
    core_chart
      .selectAll('.core-chart-line-highlight')
      // .transition()
      // .duration(60)
      .remove();
    core_paths
    ...
    legend.select('.legend-color').style('border-radius', '3px');
